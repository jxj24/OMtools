...
%Modified by: L.F. Dell'Osso
%             September 24, 2009
%Modified by: Joel Simon
%             September 25, 2009; (last mod: 10/29/09)
...
%function to draw the main window
    function draw_main()
        ...
        main_buttons(1)=uicontrol('parent',main_window,'style','pushbutton','string','Save Current',...
            'position',[10 40 100 30],'callback',@save_current,'tooltip','Save the Current OMS as an (.oms) file');
        main_buttons(2)=uicontrol('parent',main_window,'style','pushbutton','string','Load Custom',...
            'position',[10 75 100 30],'callback',@load_current,'tooltip','Load a previously saved OMS file');
        ...
        uicontrol('parent',main_window,'style','text','fontunits','pixels','fontsize',18,'position',[35 main_height-20-1 200 21],...
            'string','Current OM System','backgroundcolor',[0 0 0],'foregroundcolor',[1 1 1]);
        ...
        function main_run(hObject,eventdata)
            ...
            mdl=mdlsep{1};
            if comp(1) == 'P'
                set_param([mdl '/Eye//Tgt Scope'],'TimeRange',current.Run_time)
            else    
...
%Function to allow the user to add tool panels to custom systems
    function add_panel(hObject1,eventdata)
        ...
        set(panel_ui,'toolbar','none','resize','off','menubar','none',...
            ...
            'position',[300 300 300 30*numel(panels_to_add)+30],'color',[0 0 0]);
        ch_box=cell(numel(panels_to_add,1));
        chk_y=30*numel(panels_to_add)-50;
        for pp = 1 : numel (panels_to_add)
            ch_box{pp}=uicontrol('parent',panel_ui,'style','checkbox','units','pixels',...
                'position',[10 chk_y 270 30],'string',panels_to_add{pp},'backgroundcolor',[0 0 0],'foregroundcolor',[1 1 1]);
            chk_y=chk_y-25;
        end
        hoa=uicontrol('parent',panel_ui,'style','pushbutton','units','pixels','position',[40 30*numel(panels_to_add) 75 25],...
            'string','Add to Main','callback',@add);
        uicontrol('parent',panel_ui,'style','pushbutton','units','pixels','position',[200 30*numel(panels_to_add) 75 25],...
            'string','Cancel','callback',@cancl);
        hoe=uicontrol('parent',panel_ui,'style','pushbutton','units','pixels','position',[120 30*numel(panels_to_add) 75 25],...
...
%them to the extra panel
    function hide_panel(hObject4,eventdata)
        ...
        set(h_panel_ui,'toolbar','none','resize','off','menubar','none',...
            ...
            'position',[300 300  300 numel(panels_open)*30+30],'color',[0 0 0]);
        %create the check-boxes for each open panel
        ch_box=cell(numel(panels_open,1));
        chk_y=numel(panels_open)*30-30;
        for pp = 1 : numel (panels_open)
            ch_box{pp}=uicontrol('parent',h_panel_ui,'style','checkbox','units','pixels',...
                'position',[10 chk_y 270 30],'string',panels_open{pp},'foregroundcolor',[1 1 1],'backgroundcolor',[0 0 0]);
            chk_y=chk_y-25;
        end
        uicontrol('parent',h_panel_ui,'style','pushbutton','units','pixels','position',[40 numel(panels_open)*30 75 25],...
            'string','Hide','callback',@hide);
        uicontrol('parent',h_panel_ui,'style','pushbutton','units','pixels','position',[200 numel(panels_open)*30 75 25],...
            'string','Cancel','callback',@cancl);
        uicontrol('parent',h_panel_ui,'style','pushbutton','units','pixels','position',[120 numel(panels_open)*30 75 25],...
...
%default   Also contains a check to preventthe user from losing data
    function sel_default(hObject,eventdata)
        if strcmpi(get(currently_editing,'string'),'custom')
            ...
            uicontrol('parent',303,'style','text','string',{'You have made changes to a Custom OMS','Do you want to'},'units','pixels','pos',[5 45 390 30]);
...
%%
%Function to load a custom OMS file
    function load_current(hObject,eventdata)
        [fln,pthn]=uigetfile({'*.oms;*.mat','OMS FIles'});
...
%%
%Function to allow user to save a Custom OMS
    function save_current(hObject,eventdata)
        [fln,pth]=uiputfile({'*.oms','OMS File'},'Save Current Settings');
        if fln==0
            return
        end
        cd(pth);
...
%Function to draw tools in any window that has tools
    function draw_tools(hObject,eventdata)
        ...
        for i = (1+index_init):(size(panel_list,2)+index_init)
            ...
            num_sl_labels=numel(slid_index)-numel(strfind([data{slid_index,3}],'%%%%'))+numel(strfind([data{slid_index,3}],'%%%%%'));
...
%called by edit settings  
    function add_default(hObject,eventdata)
        ...
        def_names=['Add a New Preset OMS';def_names];
        editor4=uipanel('Parent',edit_gui,'units','pixels','Title','Add or Edit a Preset','Position',[edit_size 5 230 590]);
        uicontrol('parent',editor4,'style','text','units','pixels','position',[5 550 220 20],'string','Select an OMS to Edit');
...
%called by edit settings
    function ed_tool(hObject,eventdata)
        ...
        function set_dyn(hObject,eventdata)
            ...
            delete(del1);
            edit_table2=uitable('parent',dyn_w,'data',def_data,'position',[1 5 200 135],'columnnames',{'OM System','Default'});
            switch get(t_choice,'value')
                case 1 %switch
                    styl='switch';
                case 2 %box
...
%called by init_load and load_settings:on error
    function create_settings
        ...
        data{1, 1} = ' settings';data{1, 2} = 'Tool_Panel_ID';data{1, 3} = 'cntrl_type';
